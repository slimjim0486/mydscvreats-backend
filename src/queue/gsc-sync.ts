...
export async function runGscSync(targetDate?: string) {
  const date = targetDate ?? yesterdayIsoUtc();
  console.log(`[gsc-sync] starting sync for ${date}`);
    ...
    await Promise.all(
      batch.map(async (r) => {
        try {
          ...
          failed += 1;
          console.warn(
            `[gsc-sync] restaurant ${r.slug} failed:`,
  ...
  console.log(
    `[gsc-sync] done. processed=${processed} withData=${withData} failed=${failed} date=${date}`
  );
}
